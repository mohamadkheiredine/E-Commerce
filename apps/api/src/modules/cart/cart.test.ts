...
describe('GET /api/v1/cart', () => {
  it('starts empty with zero totals', async () => {
    ...
    expect(res.body.data).toEqual({ items: [], subtotal: 0, shipping: 0, total: 0, itemCount: 0 });
  });
});

describe('POST /api/v1/cart/items', () => {
  it('adds a variantless product', async () => {
    const res = await add({ productId: cat.plain.id, quantity: 2 });
  ...
  it('increments the existing line when the same configuration is added again', async () => {
    await add({ productId: cat.plain.id, quantity: 2 });
    const res = await add({ productId: cat.plain.id, quantity: 1 });
  ...
  it('checks stock against the combined quantity, not just the increment', async () => {
    await add({ productId: cat.plain.id, quantity: 4 });
    const res = await add({ productId: cat.plain.id, quantity: 2 }); // 6 > 5
  ...
  it('keeps different variants of the same product as separate lines', async () => {
    await add({ productId: cat.shirt.id, variantId: cat.variants.S.id, quantity: 1 });
    const res = await add({ productId: cat.shirt.id, variantId: cat.variants.M.id, quantity: 1 });
...
describe('PATCH /api/v1/cart/items/:id — quantity', () => {
  it('updates the quantity and recomputes totals', async () => {
    const added = await add({ productId: cat.plain.id, quantity: 1 });
...
describe('PATCH /api/v1/cart/items/:id — variant change', () => {
  it('switches the line to another variant when no line for it exists', async () => {
    const added = await add({ productId: cat.shirt.id, variantId: cat.variants.S.id, quantity: 2 });
    const id = added.body.data.items[0].id;

    const res = await patch(id, { variantId: cat.variants.M.id });
  ...
  it('MERGES into the existing line when the target variant is already in the cart', async () => {
    await add({ productId: cat.shirt.id, variantId: cat.variants.M.id, quantity: 3 });
    const s = await add({ productId: cat.shirt.id, variantId: cat.variants.S.id, quantity: 2 });
    const sLineId = s.body.data.items.find(
      (i: { variant: { value: string } }) => i.variant.value === 'S',
    ).id;
    expect(s.body.data.items).toHaveLength(2);

    const res = await patch(sLineId, { variantId: cat.variants.M.id });
  ...
  it('refuses a merge that would exceed the target variant stock, leaving both lines intact', async () => {
    await add({ productId: cat.shirt.id, variantId: cat.variants.S.id, quantity: 2 }); // S has 3
    const m = await add({ productId: cat.shirt.id, variantId: cat.variants.M.id, quantity: 2 });
    const mLineId = m.body.data.items.find(
      (i: { variant: { value: string } }) => i.variant.value === 'M',
    ).id;

    const res = await patch(mLineId, { variantId: cat.variants.S.id }); // 2 + 2 = 4 > 3
  ...
  it('refuses switching to a sold-out variant', async () => {
    const added = await add({ productId: cat.shirt.id, variantId: cat.variants.S.id, quantity: 1 });
    const res = await patch(added.body.data.items[0].id, { variantId: cat.variants.L.id });
    expect(res.status).toBe(409);
    expect(res.body.error.code).toBe('OUT_OF_STOCK');
  });
  ...
  it('refuses a variant from another product', async () => {
    const added = await add({ productId: cat.shirt.id, variantId: cat.variants.S.id, quantity: 1 });
    const res = await patch(added.body.data.items[0].id, { variantId: 'some-other-variant' });
    expect(res.status).toBe(400);
  ...
  it('refuses a variant change on a variantless product', async () => {
    const added = await add({ productId: cat.plain.id, quantity: 1 });
    const res = await patch(added.body.data.items[0].id, { variantId: cat.variants.M.id });
    expect(res.status).toBe(400);
  });
});
...
describe('totals', () => {
  it('charges flat shipping under the threshold and none above it', async () => {
    const small = await add({ productId: cat.plain.id, quantity: 1 }); // 9,900
    expect(small.body.data.shipping).toBe(2500);
    expect(small.body.data.total).toBe(12400);

    const large = await add({ productId: cat.shirt.id, variantId: cat.variants.M.id, quantity: 2 }); // + 60,000
