...
const add = (product_id: string) =>
  api().post('/api/v1/wishlist/items').set(authed()).send({ product_id });

describe('wishlist', () => {
  it('starts empty', async () => {
    const res = await api().get('/api/v1/wishlist').set(authed());
    expect(res.status).toBe(200);
    expect(res.body.data).toEqual({ items: [], item_count: 0 });
  });

  it('adds a product and is idempotent on repeat', async () => {
    const first = await add(cat.plain.id);
    expect(first.status).toBe(201);
    expect(first.body.data.item_count).toBe(1);

    const again = await add(cat.plain.id);
    expect(again.status).toBe(201);
    expect(again.body.data.item_count).toBe(1);
  ...
  it('moves a variantless product straight into the cart', async () => {
    ...
    expect(res.body.data.wishlist.item_count).toBe(0);
  });
  ...
  it('moves a product with variants when one is chosen', async () => {
    ...
    const res = await api()
      ...
      .set(authed())
      .send({ variant_id: cat.variants.M.id });
