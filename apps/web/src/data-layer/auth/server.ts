...
import { deserializeSession, serializeLogoutBody, type SignedInSession } from '@/serializers/auth';
...
export async function login(payload: LoginPayload): Promise<SignedInSession> {
  ...
  return deserializeSession(loginResponseSchema.parse(data));
}
...
export async function logout(refreshToken: string | undefined): Promise<void> {
  ...
  await publicApi
    .post<void>('/auth/logout', serializeLogoutBody(refreshToken))
    .catch(() => undefined);
}
